...
### STEP-1: DECON
	for roi in ${rois} ;
	do
		rm -f ${sub}-${roi}-*-bucket_REML*+tlrc*
		rm -f X_*.jpg
		rm -f ${sub}-${roi}-*-xmatrix*.1D
		rm -f 3dREMLfit.err
		...
		3dDeconvolve -input sub-${sub}_task-rest_run-01_space-MNI152NLin2009cAsym_desc-preproc_bold-clean.nii* \
	    	...
	    	-local_times \
	    	-basis_normall 1 \
	    	-stim_file 1 ${sub}-${roi}.txt \
	    	-stim_label 1 "ROI-${roi}" \
	    	-xjpeg X_norm_blur-${roi}.jpg \
	    	-x1D ${sub}-${roi}-xmatrix_norm_blur.1D

		3dREMLfit -matrix ${sub}-${roi}-xmatrix_norm_blur.1D \
      		-input sub-${sub}_task-rest_run-01_space-MNI152NLin2009cAsym_desc-preproc_bold-clean.nii* \
	      	-mask /Users/jflanner/Documents/RESTING/group_mask/${sub}-mask-2mm+tlrc \
	      	-fout \
	      	-tout \
	      	-Rbuck ${sub}-${roi}-bucket_REML_norm_blur \
	        -verb
		done
done
...
for sub in ${subjects}; do
	cd /Users/jflanner/Documents/RESTING/data_rs/sub-${sub}/
	cp *insula_*-bucket_z_REML+tlrc.* /Users/jflanner/Documents/RESTING/group_level/
done
