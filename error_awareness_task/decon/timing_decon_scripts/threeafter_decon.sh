...
for sub in ${subjects}; do
	...
	-stim_times 3 threeafter_error.1D SPMG1 -stim_label 3 threeafter_error \
	-num_glt 1 \
	-gltsym 'SYM: +nogo_correct -threeafter_error' -glt_label 1 correct-threeafter_error \
	-tout -xsave -xjpeg sub-${sub}-threeafter-xjpeg -x1D sub-${sub}-threeafter-x1D -allzero_OK \
	-bucket sub-${sub}-eat_threeafter.nii.gz

	3dREMLfit -matrix sub-${sub}-threeafter-x1D \
	 -input "sub-${sub}_scaled_run_01.nii.gz sub-${sub}_scaled_run_02.nii.gz sub-${sub}_scaled_run_03.nii.gz sub-${sub}_scaled_run_04.nii.gz sub-${sub}_scaled_run_05.nii.gz sub-${sub}_scaled_run_06.nii.gz" \
	 -mask sub-${sub}_task-errorawareness_run-01_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz \
	 -GOFORIT 3 \
	 -tout -Rbuck sub-${sub}-eat_threeafter.nii.gz_REML -Rvar sub-${sub}-eat_threeafter.nii.gz_REMLvar -verb $*
done
